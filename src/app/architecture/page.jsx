...
export default function Architecture() {
  ...
  console.log('Architecture projects:', architectureProjects?.length);

  return (
    <div className="space-y-12">
      <div className="space-y-4">
        <h1 className="text-4xl lg:text-5xl font-light tracking-wider text-black">Architecture</h1>
        <div className="w-16 h-px bg-black"></div>
        <p className="text-lg text-gray-600 font-light max-w-2xl">
          Our architectural projects span residential, urban, hospitality, and cultural spaces, 
          each designed to create meaningful connections between people and place.
        </p>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {architectureProjects.length > 0 ? architectureProjects.map((project) => {
          console.log('Architecture project:', project.title, 'Image:', project.coverImage);
          return (
                ...
                <a href={`/projects/${project.category}/${project.subcategory}/${project.slug}`} className="block">
                  <div className="w-full h-64 lg:h-72 bg-gray-200">
                    <img
                      ...
                      onLoad={() => console.log('✅ Architecture image loaded:', project.title)}
                      onError={(e) => {
                        console.log('❌ Architecture image failed:', project.title);
      ...
      {/* Modal */}
      {showInfo && (() => {
        const project = architectureProjects.find(p => p.id === showInfo);
