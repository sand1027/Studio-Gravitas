...
// Custom sidebar for objects pages
  ...
  return (
    <aside className="hidden lg:block w-64 fixed h-screen z-50 pointer-events-none">
      ...
      <nav className="overflow-hidden pointer-events-auto">
        ...
        <a href="/objects" className="block py-1 pl-6 text-xs font-normal transition-colors text-white opacity-100 no-underline">Objects</a>
  ...
  useEffect(() => {
    const fetchObjects = async () => {
      try {
        ...
        const objectsProjects = data.projects?.filter(p => p.category === 'objects') || [];
        const allImages = objectsProjects.flatMap(project => 
      ...
      <div className="relative w-full h-screen bg-black">
        ...
        <div className="flex items-center justify-center h-screen">
          <div className="text-white text-center">
            <h2 className="text-2xl font-light mb-4">No Objects Available</h2>
            <p className="text-gray-400">Upload some objects to see them here.</p>
          </div>
        </div>
      </div>
    );
