...
// Custom sidebar for thoughts pages
  ...
  return (
    <aside className="hidden lg:block w-64 fixed h-screen z-50 pointer-events-none">
      ...
      <nav className="overflow-hidden pointer-events-auto">
        ...
        <a href="/thoughts" className="block py-1 pl-6 text-xs font-normal transition-colors text-white opacity-100 no-underline">Thoughts</a>
  ...
  useEffect(() => {
    const fetchThoughts = async () => {
      try {
        ...
        const thoughtsProjects = data.projects?.filter(p => p.category === 'thoughts') || [];
        const allImages = thoughtsProjects.flatMap(project => 
      ...
      <div className="relative w-full h-screen bg-black">
        ...
        <div className="flex items-center justify-center h-screen">
          <div className="text-white text-center">
            <h2 className="text-2xl font-light mb-4">No Thoughts Available</h2>
            <p className="text-gray-400">Upload some thoughts to see them here.</p>
          </div>
        </div>
      </div>
    );
